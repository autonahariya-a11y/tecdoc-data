...
(function () {
  ...
  function getProductPageData() {
    ...
    var priceEl = document.querySelector('.price_current') || document.querySelector('.price .number') || document.querySelector('.price_val') || document.querySelector('.item_current_price');
    if (!priceEl) return null;

    var data = {};

    /* Title */
    var titleEl = document.querySelector('.item_info h1') || document.querySelector('.item_title h1') || document.querySelector('h1.product_name') || document.querySelector('h1');
  ...
  function injectProductPage(pageData) {
    ...
    var itemMain = document.querySelector('.item_page_main');
    var insertTarget = itemMain || document.querySelector('.item-page') || document.querySelector('#item_main');
    if (!insertTarget) {
      insertTarget = document.querySelector('#content') || document.querySelector('main') || document.querySelector('#bg_middle');
    }
    if (!insertTarget) return false;

    /* Hide ALL original product sections */
    var hideSelectors = [
      '.item_page_main', '.item_right', '.item_left', '.item_image',
      '.item_info', '.item_info_top', '.price_area', '.add_to_cart_area',
      '.quantity_and_cart', '.buy_now', '.why_buy', '.description',
      '#item_content', '#item_specifications',
      '.image_gallery'
    ];
    ...
    /* Insert BEFORE the hidden item_page_main (or at top of content) */
    if (itemMain && itemMain.parentNode) {
      itemMain.parentNode.insertBefore(container, itemMain);
